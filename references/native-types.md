...
| `lark-uml:sequence` | `composite_shape` (Page/Window first column, `:{Business}Controller` / `:{Business}Service` / `:{Business}Dao` system layers, `DB` storage), `life_line` (lifeline tail), `life_line` / `activation` (activation bar), `combined_fragment` (alt / opt / loop / par), `connector` (message / self-call), `note_shape` (side note) | Faking a fragment with a plain rectangle; using `connector` as a lifeline; coordinate-only message endpoints; diagonal messages; stacking multiple frames instead of multi-row `combined_fragment`; using `actor` for the first column (use window/page `composite_shape` instead); using framework-specific names like `XxxMapper` for Dao (use `Dao` suffix). |
