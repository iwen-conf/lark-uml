...
| `lark-uml:sequence` | `composite_shape` (participant head, object) **or** `actor` / `composite_shape{actor}` (human), `life_line` (lifeline tail), `life_line` / `activation` (activation bar), `combined_fragment` (alt / opt / loop / par), `connector` (message / self-call), `note_shape` (side note) | Faking a fragment with a plain rectangle; using `connector` as a lifeline; coordinate-only message endpoints; diagonal messages; stacking multiple frames instead of multi-row `combined_fragment`. |
