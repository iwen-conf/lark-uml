...
- **Participant naming.** The first column is the UI entry point — a page or window name (e.g. `签到页`, `Browser`, `QAPage`), using a window/page native shape, **not** an actor stick-figure. System-layer participants use the `:{Business}{Layer}` format with a colon prefix and the business domain abbreviation followed by the layer name (e.g. `:EmotionController`, `:EmotionService`, `:EmotionDao`). The storage layer is simply `DB`. **Never** use framework-specific class names like `XxxRecordMapper` or `XxxMapper` for the Dao layer — the layer suffix is always `Dao`.
- **Default layer order (left to right).** Page/Window → `:{Business}Controller` → `:{Business}Service` → `:{Business}Dao` → `DB`. This mirrors the standard layered architecture: Controllers → Service → Dao → DB. The `{Business}` prefix is the short business domain name (e.g. `Emotion`, `Match`, `Chat`, `Ai`, `Social`). Derive both the business prefix and the method names from the system's actual source code.
...
- **Forward message structure.** The forward call chain consists of exactly five messages progressing left-to-right across adjacent lifelines:
  1. Input parameters from the Page to the Controller (e.g. `"1: moodTag, intensity, content"`)
  ...
  4. Dao method with parameter type (e.g. `"4: insert(BizEmotionRecord)"`)
  5. SQL statement executed against DB (e.g. `"5: INSERT INTO biz_emotion_record (user_id, tag_id, note)"`)
...
- **Return message structure.** Return messages follow the reverse path right-to-left, each labeled with the return type:
  6. DB → Dao: return type (e.g. `"6: int"`)
  7. Dao → Service: return type (e.g. `"7: CheckinResponse"`)
  8. Service → Controller: return type (e.g. `"8: CheckinResponse{checkinId, date}"`)
  9. Controller → Page: wrapped result (e.g. `"9: Result<CheckinResponse>"`)
- **Activation bar discipline.** Every participant column **except the first Page/Window column** gets an activation bar. The activation bar starts at the first incoming message to that participant and ends at the last return message from that participant. The Page column never carries an activation bar.
- **One round trip only.** The diagram shows exactly one forward call chain (messages 1–5) and one return chain (messages 6–9). Do not add extra acknowledgment messages, intermediate processing steps, or additional "render / display" messages beyond the single return to the Page. When branching logic is required, use a `combined_fragment` (`alt` / `opt`) around the relevant messages — but each branch still follows the same numbered forward-then-return structure internally.
...
- **Participant header** — The first column uses a window/page shape (`composite_shape` with rounded-rect style, e.g. `签到页`, `Browser`). System-layer headers use the `:{Business}{Layer}` format with a colon prefix (e.g. `:EmotionController`, `:EmotionService`, `:EmotionDao`). The storage layer is simply `DB`. All headers share the same top `y` and uniform spacing. Headers are produced by cloning an existing header of the same kind.
...
1. **Participant head** → First column: window/page `composite_shape` (plain name, e.g. `签到页`). System layers: `composite_shape` labeled `:{Business}{Layer}` (e.g. `:EmotionController`). Storage: `composite_shape` labeled `DB`. Never `round_rect` / `text_shape` / `actor` directly.
