...
- **ER exception:** `lark-uml:er` is raw-first and native-only because table rows, FK markers, connector captions, one-line-per-table-pair rules, and schema verification require direct native `table_uml` / `connector` control.
...
`lark-uml:er` is scoped to physical database structure. Before drawing, it must derive the final table / field inventory from the real schema source such as migrations or DDL, including later `ALTER TABLE` migrations. The finished board must have the same tables and fields as the schema, native straight connectors, at most one visible connector per table pair, cardinality in native arrow styles, optional visible cardinality text only as connector captions, and a compact low-crossing layout.
...
3. **Default route:** PlantUML source is piped to `lark-cli whiteboard +update --input_format plantuml --overwrite`. The Feishu parser generates editable native nodes.
   For `lark-uml:er`, use raw native editing instead of PlantUML so schema rows, connector captions, and one-line-per-table-pair validation can be enforced.
